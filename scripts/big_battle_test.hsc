...
(script dormant sc_marine_test
	(ai_place sq_elite01 3)
	(ai_place sq_marine02)
)
		...
		(begin
			(sleep_until 
				(and
					...
					(= (ai_task_count obj_bugger/main_gate) 0) 
					(= (ai_task_count obj_inside/main_gate) 0) 
					(= (ai_task_count obj_skirmisher/main_gate) 0)
					(= (ai_task_count obj_spartan/main_gate) 0)					
					(= b_spartan 0)
				)	
			1)
				(if (= s_spawn 1)
							...
							((= s_weapon_type 10)
								(begin
									(cond
										((= s_character_type 0)
											(begin
												(print "No Minor Plasma Launcher")
												(if (= b_character 1)
													(set s_character_type (+ s_character_type 1))
												)	
											)
...
(script static void perception
	(cs_run_command_script sg_ai cs_perception)
	(sleep 30)
	(sleep_until (> (ai_combat_status sg_ai) 5) )
		(cs_run_command_script sg_ai cs_abort)
)

(script command_script cs_perception
	(sleep_until
		(begin 
			(cs_force_combat_status 1)
			(cs_go_to_and_face ps_perception/p0 ps_perception/p1)
		0)
	1)
)
...
(script command_script cs_skirmisher
	(cs_force_combat_status 3)	
	(cs_enable_pathfinding_failsafe TRUE)
	(cs_walk FALSE)
	(sleep_until
		(begin
			(cs_go_to ps_skirmisher/p0 1)
			(cs_go_to ps_skirmisher/p1 1)
		0)
	1)
)
